...
import { setNavPlace,setUserImage,setCountryName,setStateName,setCityName,setHotelType,setHotelTypeId } from '../../../redux/actions';
 ...
 const UpdateAcount = async() => {
  ...
  const GetAcountDetail=async() => {
    ...
    .then(function (response) {
      ...
      dispatch(setHotelType(response.data[0].hotel_id[0].hotel_name))
      dispatch(setHotelTypeId(response.data[0].hotel_id[0]._id))
     //setImage(BASE_URL+response.data[0].img)
      ...
      setChecked(response.data[0].gender)
    })
    .catch(function (error) {
      console.log("error", error)
    })
